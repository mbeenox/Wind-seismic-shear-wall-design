...
*Updated: 2026-06-16 (rev 29 — **DEPLOYED TO VERCEL. A Vite host scaffold was added so the app builds + serves as a static site: `index.html` · `main.jsx` · `package.json` · `vite.config.js` · `.gitignore`. The app jsx is BYTE-IDENTICAL / UNCHANGED — no app-code change, engine + the 7 guarded fns untouched, `APP_BUILD` stays 110 / "Version 1.10". Scaffold lives in the host shell, NOT in the app file. See the rev-29 blockquote below + §4dd for the exact files + repo layout. The two-story feature's live sign-off (preview the stacked 1st-floor detailing + a two-story `.wps` save/open on the live Vercel URL) is STILL the open item.** Prior current rev — rev 28 — **two-story wood shear wall design — STEP 7 COMPLETE; the feature (Steps 1–7) is feature-complete pending the user's live sign-off**; see the rev-28 blockquote below and **§4z**/**§4aa**/**§4bb**/**§4cc** for the full feature log. Step 7a = the secondary detailing (anchor · embed · strap · deflection · footing) is re-derived from the stacked demand; Step 7c = a 25-assert stacking regression test; Step 7b = a frozen two-story `.wps` fixture + 31-assert round-trip load test + a schema tripwire locking the two-story field names (`H2`, `twoStory`, `activeFloor`, `linesByFloor`). All test/fixture infra is checked in; 7b made NO app-code change (persistence already worked). Engine + the 7 guarded fns byte-identical throughout.) · File: `plan-sketcher-suite.jsx` (~4,036 lines, single file, default export `App`)*
...
- **Ribbon toolbar** (in PlanSketcher, above canvas): File (New/Open/Save), Edit (Undo/**Redo**), Draft (✏ Draw / ⌗ Snap / ∟ Ortho / ⟷ Dims toggles), Presets, Analyze (⚡ Design shear walls — disabled until reactions exist). Side-panel cards intentionally duplicated, not removed.
