...
*Updated: 2026-06-25 · **rev 63 / APP_BUILD 142**. Current frontier: PLAN SKETCHER UX. rev 63 is a Plan-tab drafting/UX pass (NOT the seismic-stacking work that the old handoff had pencilled into the rev-63 slot — that is deferred to **rev 64**): (1) the minimum drawable/adjustable wall length drops 1 ft → **0.5 ft** (`MIN_DRAW_FT`); (2) dropping a node onto the **body of another wall now auto-splits** that wall at the landing point and welds the node there (T-intersection bind — move it later and all three walls follow); (3) the **plan/level switcher** moves from below the canvas to floated **top-center inside the canvas**, and all of its "floor" wording becomes "**Level**" ("Level 1"/"Level 2" buttons, "Level N Plan" badge). `calcCore.js` BYTE-IDENTICAL (pure UI/geometry rev, no engine touch). Next (rev 64): 2-story seismic stacking + the upper-story-DL correction (the deferred rev-63 plan). Full detail: §4zzl.*
...
**Where rev 63 left off (PLAN SKETCHER UX — min length · T-split · level switcher):** a Plan-tab drafting/UX pass, **no engine change** (`calcCore.js` BYTE-IDENTICAL). Three changes, all in `plan-sketcher-suite.jsx`:
  1. **Min draw length 1 ft → 0.5 ft.** New module-level const `const MIN_DRAW_FT = 0.5;` (next to `VB_W/VB_H/GRID/PAD`). Enforced in `applyWallLength` (`if(!(newLen>=MIN_DRAW_FT)) return;`, was `>0`) and the dim-editor click-away commit (`if(v>=MIN_DRAW_FT)…`). The dim input is now `min={MIN_DRAW_FT} step="0.5"`, and the editor OPENS at 1-decimal precision (`val:String(Math.round(dist(a,b)*10)/10)`, was integer) so half-foot lengths display/edit cleanly. Free-draw resolution was already 0.1 ft (snap-off `Math.round(v*10)/10`), so drawing fine lengths needed no change — the perceived "1 ft floor" was purely the dim input's old `min="1"`.
  2. **T-intersection auto-split.** New `tryBindTJunction(nodeId)` (a `useCallback`, deps `[snapshot]`, placed just before `onMove`), called from `onUp`'s nodeDrag branch only when `nd.moved`. On drop it scans every edge NOT already incident to the node, projects the node onto each (param `t`, foot `fx/fy`), and accepts the nearest whose perpendicular distance `≤ tol=1.6·SRef.current` AND whose foot is INTERIOR (`t∈(0,1)` and ≥ `endMargin=max(tol,MIN_DRAW_FT)` from each end). It then `snapshot()`s, **welds** the node to the foot, and **splits** the target edge `a–b` into `norm(a,node)`+`norm(node,b)` (de-duped via `same`). Net effect: the dragged node becomes the shared T-node, so a later drag of it moves all three walls. Endpoint joins, far drops, and degenerate splits are skipped. (`onUp` deps gained `tryBindTJunction`.)
  3. **Plan/level switcher relocated + renamed.** The `.floorbar` block moved OUT of the canvas-column-bottom and INTO `.stage` (right after the floor badge). CSS: `.floorbar` is now `position:absolute;top:8px;left:50%;transform:translateX(-50%);z-index:6;pointer-events:none;` and `.floorsel` gained `pointer-events:auto` (so only the buttons catch clicks — the rest of the canvas stays drawable). Buttons "1st Floor"/"2nd Floor" → "**Level 1**"/"**Level 2**"; the corner badge "Floor N plan" → "**Level N Plan**". Disabled/greyed-until-2-Story behavior is UNCHANGED. NOTE — the per-wall DL-tributary window's "1st floor"/"2nd floor" tags (≈line 1502) and the Design-tab floor labels (≈line 4558) are a DIFFERENT surface and were intentionally left as-is (the task scoped only the Plan Switcher).
  `APP_BUILD=142`. **Deploy: replace `src/plan-sketcher-suite.jsx` ONLY — `calcCore.js` UNCHANGED.** Verified this session: `calcCore.js` byte-identical (diff vs the uploaded original); modular esbuild compile CLEAN; SSR render smoke 4/4 (Level 1 + Level 2 present, no "1st Floor" switcher, 37,076 chars); T-junction geometry unit test 12/12 (midpoint splits + welds to the wall line; new edges `1-3`/`2-3` present, target removed, edge count 2→3; near-endpoint / too-far / already-incident all correctly DON'T split; vertical-wall T splits + welds to `x`). **Next:** rev 64 — the DEFERRED 2-story seismic stacking (see §4zzk for the plan: per-floor `F_roof`/`F_floor` into `stackSeg`, upper-DL to both uplift-resist + post-compression, cumulative footing base-shear; guarded change, approval on record). See §4zzl.
...
**Open items to pick from (full detail in §7):** **rev 64 (next, the deferred rev-63 plan): 2-story seismic stacking** (§4zzk lists the mechanism) · PDF/report export · footings UI placement · per-line design constraints (currently global) · mobile ergonomics of on-diagram inputs (the rev-63 T-split helps desktop drafting, but touch pinch/pan is still open) · dash highlight only on the selected design line · `.wps` stores **inputs+geometry+session-UI** and is now forward-compatible against ADDED fields across `g`/`d`/`segments`/per-wall props/`lines`/placed segments (rev 23–24), but still stores no **derived results / optimizer `meta`** — optionally persist those for an engine-independent archive · **renames + unit/semantics changes are still NOT auto-handled** — they need an explicit `MIGRATIONS` step + a frozen old-version fixture; the schema tripwire (`test_schema.cjs`) + the migration checklist (§7) enforce the discipline.
...
## 4zzl. Rev 63 — Plan Sketcher UX: 0.5 ft min length · T-intersection auto-split · level switcher relocated + renamed (user-requested)

A Plan-tab drafting/UX pass requested by the user. **No engine change** — `calcCore.js` BYTE-IDENTICAL (verified by diff vs the uploaded original); all edits live in `plan-sketcher-suite.jsx` outside every guarded fn. This rev took the slot the old handoff had pencilled for 2-story seismic stacking; that work is **deferred to rev 64** (the plan in §4zzk's "open" notes still stands).

**(1) Minimum draw/adjust length 1 ft → 0.5 ft.** New module-level const `const MIN_DRAW_FT = 0.5;` (beside `VB_W/VB_H/GRID/PAD`). The old "1 ft floor" was *only* the dimension input's `min="1"` attribute — free-draw resolution was already 0.1 ft (snap-off `Math.round(v*10)/10`), so drawing fine lengths never needed a change. Edits: `applyWallLength` guard `if(!(newLen>0))` → `if(!(newLen>=MIN_DRAW_FT))`; the dim-editor click-away commit `if(v>0)` → `if(v>=MIN_DRAW_FT)`; the dim `<input>` is now `min={MIN_DRAW_FT} step="0.5"`; and the editor OPENS at 1-decimal precision (`val:String(Math.round(dist(a,b)*10)/10)`, was integer `Math.round(dist)`) so a 12.5 ft wall reads "12.5" instead of "13". Sub-0.5 entries are rejected (no apply), matching "minimum allowable length."

**(2) T-intersection auto-split + bind.** New `tryBindTJunction(nodeId)` — a `useCallback` (deps `[snapshot]`) placed just before `onMove`; called from `onUp`'s nodeDrag branch **only when the node actually moved** (`if(nd.moved) tryBindTJunction(nd.id);`). Algorithm: scan every edge NOT already incident to `nodeId`; for each, project the node onto the segment (param `t`, foot `fx/fy`); accept the nearest candidate whose perpendicular distance `pd ≤ tol = 1.6·SRef.current` (zoom-aware) AND whose foot is INTERIOR — `t∈(0,1)` and at least `endMargin = max(tol, MIN_DRAW_FT)` from each endpoint (so endpoint joins and degenerate near-zero splits are excluded). On a hit: `snapshot()`, then in one `setGraph` (a) **weld** the node to the foot (`{...n, x:fx, y:fy}`) and (b) **split** the target edge `a–b` into `norm(a,nodeId)` + `norm(nodeId,b)`, filtering the original via `same` and de-duping. Net topology: the dragged node becomes the shared T-node, so a later drag of it moves all three walls together (the incoming wall + both split halves). Far drops, endpoint-to-endpoint landings, and walls already touching the node are all left alone. `onUp` deps gained `tryBindTJunction`. (Reuses the existing module helpers `norm`/`same`; no new global state.)

**(3) Plan/level switcher relocated + renamed.** The `.floorbar` block moved out of the canvas-column bottom (outside `.stage`) to INSIDE `.stage`, immediately after the floor badge, so it floats at the top of the drawing area. CSS rewritten: `.floorbar` is now `position:absolute; top:8px; left:50%; transform:translateX(-50%); z-index:6; pointer-events:none;` and `.floorsel` gained `pointer-events:auto` — so the bar is click-through and **only the buttons** catch pointer events, leaving the rest of the canvas drawable beneath it (the box-shadow was also lightened to a drop shadow for the on-canvas context). Labels: buttons "1st Floor"/"2nd Floor" → "**Level 1**"/"**Level 2**"; the corner badge `Floor {activeFloor} <span>plan</span>` → `Level {activeFloor} <span>Plan</span>` (CSS uppercases it to "LEVEL N PLAN"). The greyed/disabled-until-2-Story behavior and the badge's 2-Story-only visibility are UNCHANGED. **Intentionally NOT touched** (out of the task's scope, a different surface): the per-wall DL-tributary window's "1st floor"/"2nd floor" tags (~L1502) and the Design-tab `{f===1?"1st Floor":"2nd Floor"}` labels (~L4558).

**Verification.** `calcCore.js` byte-identical (diff CLEAN vs the uploaded original). Modular esbuild compile CLEAN. SSR render smoke **4/4** (`renderToString(App)` contains "Level 1" + "Level 2", does NOT contain the old "1st Floor" switcher label, length 37,076). T-junction geometry unit test **12/12** (standalone replica of the foot-projection + interior gate + split): midpoint drop splits and welds the node onto the wall line; new edges `1-3`/`2-3` present, original `1-2` removed, edge count 2→3, incoming `3-4` intact; near-endpoint / too-far / already-incident drops correctly do NOT split; a vertical target wall splits and welds to its `x`. `APP_BUILD=142`. **Deploy: replace `src/plan-sketcher-suite.jsx` ONLY — `calcCore.js` UNCHANGED.** (Full §6b sweep not run — focused guard only, per §0; the harness files weren't in the upload. When the harness is regenerated, add an interaction test that drags a node onto a wall body and asserts the 2→3 edge split + shared-node weld.)
...
- **Seismic in the Design tab (rev 62, §4zzk):** DONE for 1-story — each line carries `forceLbsSeismic` (computed in `computeHandoff`, view-independent), fed to `lineResults`+optimizer as `vSeismic`; the engine's native envelope designs each line for the heavier case; per-element W/S badges (`_govShearCase`/`_govBy`/`CaseTag`) + `S_DS` on the Design Seismic card. `calcCore.js` untouched. **Open:** (a) **rev 63 next** — 2-story seismic stacking: feed per-floor `F_roof`/`F_floor` into `stackSeg`, add the **upper-story DL as stabilizing to BOTH uplift-resist and post-compression** via each case's factored bucket (`+r2.BwDL` seismic uplift, `+r2.CwDL` wind uplift, `+r2.AwDL` seismic comp, `+r2.wdl` wind comp — all already on `r2`), and make the footing base-shear **cumulative** (`r1.Fs+r2.Fs`, `r1.Fw+r2.Fw`); guarded `stackSeg`/`stackedLineResults` change, approval on record, golden re-baseline + regression vs the rev-60 2-story Section-B targets. (b) "Send line to calculation sheet" still uses the GLOBAL manual `g.vSeismic` (no per-tab seismic) — a seismic-governed Design line won't reflect that on the calc sheet until a per-tab-seismic rev. (c) schema tripwire should add `forceLbsSeismic` to `schema.expected.json` when the harness is regenerated (additive, no migration).
...
- **Plan Sketcher UX (rev 63, §4zzl):** DONE — (1) min draw/adjust length is now `MIN_DRAW_FT = 0.5` ft (was a 1-ft floor from the dim input's `min` only); (2) dropping a node on the body of another wall auto-splits that wall and welds the node (`tryBindTJunction`, on `onUp`); (3) the level switcher floats top-center inside the canvas with "Level 1"/"Level 2" buttons + a "Level N Plan" badge. `calcCore.js` untouched. **Judgment calls / open:** (a) the T-bind fires on **drop** (`onUp`), not live during the drag — a live "rests-on-the-wall" preview ring/highlight while dragging is a possible polish follow-up; the perpendicular tolerance is `1.6·S` (zoom-aware) — widen/narrow if it feels grabby or too strict. (b) The bind welds to the perpendicular **foot** (ignores grid snap at the moment of binding — landing ON the wall beats landing on grid, by design); revisit if a grid-locked T is ever wanted. (c) The split does NOT re-merge if the node is later dragged off the wall — once split, the target wall stays two segments (matches the old manual workaround's result; add an auto-heal only if requested). (d) The switcher stays visible-but-greyed in 1-story mode (unchanged behavior, just relocated); hiding it entirely until 2-Story is on is a one-line change if the floating greyed control feels like clutter. (e) The DL-tributary window + Design-tab floor labels still say "1st/2nd floor" — intentionally left (different surface); rename those too if the "Level" terminology should be global.
