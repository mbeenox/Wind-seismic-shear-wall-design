...
*Updated: 2026-06-25 · **rev 61 / APP_BUILD 140**. Current frontier: SEISMIC → DESIGN TAB. rev 61 is the prerequisite engine-convention change — dropped the `/R` from `calcSegment`'s seismic ASD conversion so `g.vSeismic` is now the post-R (ASCE 7 reduced) base shear, parallel to `wWind`. Next (rev 62): wire the per-line seismic reaction into the Design-tab demand. Full detail: §4zzj.*
...
- **Seismic engine convention (rev 61, §4zzj):** `g.vSeismic` is now the **post-R reduced** seismic base shear (lbs), not the old "lbs·R" un-reduced value — the engine dropped its `/R` and applies only ×0.7, mirroring wind. `g.R` is reference-only now (kept in `g`/save files, shown on the calc sheet, unused by the math). The `calcSegment` guard is golden-OUTPUT (re-baselined). This was the prerequisite to feed the Design tab; **rev 62 is next** — wire each line's seismic reaction in as `vSeismic`. KEY ENGINE FACTS confirmed this rev (drive rev 62/63 design): (a) the optimizer is **already case-aware** — `evaluateCandidate` picks `type = max(sugS, sugW)`; (b) `calcSegment` **already envelopes every element** across wind/seismic (`maxComp`/`maxUplift`/`maxStrap`/`reqFtgLen` via `xMax`, status via `max(sugS,sugW)`) — so the per-element "Option B" envelope is native, not new code; (c) E_v is in the math via `g.sds` (compression `A=1+0.14·sds`, uplift `B=0.6−0.14·sds`) — shared global, calc-sheet input, to be surfaced on the Design Seismic card in rev 62; (d) the only reason no Design line reads seismic-controlled today is `lineResults` feeds only `wWind`, leaving `vSeismic` at the global default. RESOLVED for the upcoming work: per-element governing notation (not a single line badge); two-story will add the **upper-story DL as stabilizing to BOTH uplift-resist and post-compression** through each case's factored bucket (`+r2.BwDL` seismic uplift, `+r2.CwDL` wind uplift, `+r2.AwDL` seismic comp, `+r2.wdl` wind comp — all already on `r2`), AND make the footing base-shear cumulative (`r1.Fs+r2.Fs`, `r1.Fw+r2.Fw`) — both are guarded `stackSeg`/`stackedLineResults` changes (rev 63, approval on record).
