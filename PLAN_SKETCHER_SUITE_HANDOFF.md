...
*Updated: 2026-06-25 · **rev 63 / APP_BUILD 142**. Current frontier: SEISMIC COMPLETE through 2-story. rev 63 wires per-floor seismic (`F_roof`/`F_floor`) into the 2-story stacked design and, as a sanctioned guarded `stackSeg` change, adds the **upper-story dead load** as stabilizing to BOTH uplift-resist and post-compression (each via its factored bucket) and makes the footing base-shear cumulative. Wind+seismic are now enveloped per element on both 1- and 2-story. Full detail: §4zzl.*
