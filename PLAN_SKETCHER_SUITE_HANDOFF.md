...
*Updated: 2026-06-16 (rev 31 — **PLAN GLYPHS HALVED.** Per user request, three on-plan drawing elements are now half their former size: the **wall nodes** (visible dot + selection halo + stroke — the transparent click/grab hit-target is deliberately kept full size), the **point-load (reaction) arrow** (shaft + stroke; its arrowhead auto-halves via `markerUnits="strokeWidth"`), and the **plf line-load arrows** (standoff length + tip + baseline/arrow strokes; arrowhead auto-halves likewise). Pure SVG-attribute change; no state/control added (a definite resize, not a knob). Engine + the guarded fns byte-identical (all 10 verified identical). `APP_BUILD = 112` ("Version 1.12"). See the rev-31 blockquote + §4ff.** Prior current rev — rev 30 — **TEXT SCALE control** (View ▸ Text dropdown, `1×`/`0.75×`/`0.5×`/`0.25×`) shrinks on-plan label text; see the rev-30 blockquote + §4ee. Step 7a = the secondary detailing (anchor · embed · strap · deflection · footing) is re-derived from the stacked demand; Step 7c = a 25-assert stacking regression test; Step 7b = a frozen two-story `.wps` fixture + 31-assert round-trip load test + a schema tripwire locking the two-story field names (`H2`, `twoStory`, `activeFloor`, `linesByFloor`). All test/fixture infra is checked in; 7b made NO app-code change (persistence already worked). Engine + the 7 guarded fns byte-identical throughout.) · File: `plan-sketcher-suite.jsx` (~4,036 lines, single file, default export `App`)*
...
- **Plan glyph sizes halved (rev 31, §4ff):** nodes, the reaction/point-load arrow, and the plf line-load arrows are now half their pre-rev-31 size (literal multipliers, `rev 31:`-commented). The node **hit-target stays full size** (`3.5·S`) so clicking isn't harder. Arrowheads scale via `markerUnits="strokeWidth"` (no marker edits). Reversible; promote to a `PLAN_GLYPH` constant or a control only if the user wants adjustability.
