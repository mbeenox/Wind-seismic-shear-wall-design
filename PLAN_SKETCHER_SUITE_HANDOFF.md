...
*Updated: 2026-06-25 · **rev 64 / APP_BUILD 143**. Current frontier: PLAN-SKETCHER DRAFTING UX (no engine change). rev 64 lands three Plan-tab improvements: (1) drafting precision down to **0.5 ft** — finest grid/snap step is now 0.5 (was 1), the length editor accepts 0.5-ft values, and dimension labels read in 0.5-ft increments (10.5 ft no longer rounds to 11); (2) **auto-split on a T-intersection** — dropping a dragged node on the body of another wall now snaps it exactly onto that wall, splits the wall in two, and binds the node so later moves drag all three walls together (mirrors the manual split's prop/flag propagation — same end-topology, automated); (3) the **plan switcher moved above the canvas** and is relabeled **Level 1 / Level 2**, with the in-canvas indicator now **Level N Plan**. `calcCore.js` + every guarded fn BYTE-IDENTICAL (all changes are UI/geometry/helpers). Full detail: §4zzm.*
